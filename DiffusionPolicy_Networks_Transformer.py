...
class WindowedCausalAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        window_mask = torch.ones(N, N, device=x.device, dtype=torch.bool)
        for i in range(N):
            start = max(0, i - self.window_size + 1)
            end = i + 1
            window_mask[i, start:end:self.stride] = False
        
        if mask is not None:
            window_mask = window_mask.logical_or(mask.bool())
        
        window_mask = window_mask.float().masked_fill(window_mask == 1, float('-inf')).masked_fill(window_mask == 0, float(0.0))
...
# progressive causal attention
class ProgressiveTransformerDecoderLayer(nn.Module):
    def __init__(
        self, 
        d_model: int, 
        nhead: int, 
        ...
        layer_idx: int = 0,
        num_window_layers: int = 6, 
        window_size: int = 4,
        stride: int = 2
    ):
        super().__init__()
        self.layer_idx = layer_idx
        self.num_window_layers = num_window_layers
        
        if layer_idx < num_window_layers:
            self.self_attn = WindowedCausalAttention(
                dim=d_model,
                num_heads=nhead,
    ...
    def _sa_block(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor]) -> torch.Tensor:
        if self.layer_idx < self.num_window_layers:
            x = self.self_attn(x)
        else:
...
class TransformerForDiffusion_D(ModuleAttrMixin):
    def __init__(self,
            input_dim: int,
            output_dim: int,
            horizon: int,
            ...
            n_cond_layers: int = 0,
            num_window_layers: int = 6,  # 前6层用窗口注意力
            window_size: int = 4,
            stride: int = 2
        ) -> None:
        super().__init__()
        ...
        if obs_as_cond:
            self.cond_obs_emb = DepthwiseSeparableConv1d(
        ...
        if T_cond > 0:
            ...
            decoder_layers = nn.ModuleList([
                ProgressiveTransformerDecoderLayer(
                    d_model=n_emb,
                    nhead=n_head,
                    dim_feedforward=4*n_emb,
                    dropout=p_drop_attn,
                    ...
                    norm_first=True,
                    layer_idx=i,
                    num_window_layers=num_window_layers,
                    window_size=window_size,
                    stride=stride
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, TransformerForDiffusion_D):
    ...
    def get_optim_groups(self, weight_decay: float=1e-3):
        ...
        decay = set()
        no_decay = set()
        whitelist_weight_modules = (torch.nn.Linear, torch.nn.MultiheadAttention, nn.Conv1d)
...
def test1():
    ...
    transformer = TransformerForDiffusion_D(
        input_dim=16,
        output_dim=16,
        horizon=8,
        n_obs_steps=4,
    ...
    print("Test 2 passed, output shape:", out.shape)
    ...
    cond = torch.zeros((4,4,10))
    out = transformer(sample, timestep, cond)
    print("Test 3 passed, output shape:", out.shape)
    ...
    sample = torch.zeros((4,8,16))
    out = transformer(sample, timestep)
    print("Test 4 passed, output shape:", out.shape)
